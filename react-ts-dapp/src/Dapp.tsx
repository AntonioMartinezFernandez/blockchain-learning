...
export class Dapp extends Component<object, DappState> {
  private _publicClient?: PublicClient;
  private _walletClient?: WalletClient;
  private _tokenContract?: ReturnType<typeof getContract>;
  private _pollDataInterval?: ReturnType<typeof setInterval>;
  ...
  render() {
    ...
    if (!this.state.selectedAddress) {
      return (
        <ConnectWallet
          ...
          networkError={this.state.networkError ?? null}
    ...
    return (
      <div className="container p-4">
          ...
          <div className="col-12">
            ...
            {this.state.transactionError && (
              <TransactionErrorMessage
                message={this._getRpcErrorMessage(this.state.transactionError) as string}
        ...
        <div className="row">
          <div className="col-12">
            {this.state.balance === BigInt(0) && <NoTokensMessage selectedAddress={this.state.selectedAddress} />}
  ...
  private _initializeViem(tokenAbi: any, tokenAddress: string) {
  ...
  private async _updateBalance() {
    ...
    const balance = (await this._tokenContract.read.balanceOf([this.state.selectedAddress])) as bigint;
  ...
  private async _checkNetwork() {
    const currentChainId = Number(await window.ethereum!.request({ method: 'eth_chainId' })).toString();
